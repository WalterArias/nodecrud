...
route.put("/api/articulos/:codigo", (req, res) => {
  ...
  let sql =
    "UPDATE articulo SET  descripcion = ?, precio = ?, existencia = ? WHERE codigo = ?";
  conexion.query(
    sql,
    [descripcion, precio, existencia, codigo],
    (error, resultado) => {
      if (error) {
        throw error;
      } else {
...
//inserta un usuario con pwd hashing
route.post("/api/usuario/", (req, res) => {
  const salt = bcrypt.genSalt(10);
  const pwdHashing = bcrypt.hash(req.body.pwd, salt)
